...
var regexMatch = (function (regex, value) {
    return value.match(regex) || [];
});

var urlMarker = {
    mark: function mark(url) {
        var randID = Math.random() + Date.now();
        return url + "?x-div-id" + randID;
    },
    getMark: function getMark(url) {
        return regexMatch(/x-div-id([0-9]+\.[0-9]+)/, url)[1];
    }
};
...
function getControllerSrc(controllerSrc) {
    var src = /\.js$/i.test(controllerSrc) ? controllerSrc : controllerSrc + ".js";
...
var getCurrentScript = function getCurrentScript() {
    ...
    try {
        ...
        var scriptMark = regexMatch(/\sat\s.+?(\n|$)/g, stack).map(urlMarker.getMark).filter(function (v) {
            return !!v;
        })[0];
        ...
        var currentScript = [].slice.call(scripts).filter(function (scriptEl) {
            var scriptSrc = scriptEl.getAttribute("src") || "";
            var elMark = urlMarker.getMark(scriptSrc);
...
var xDivTester = function () {
  function xDivTester() {
    classCallCheck(this, xDivTester);
  }

  createClass(xDivTester, [{
    key: 'contructor',
    ...
    value: function callWith(el) {
      assert((typeof el === 'undefined' ? 'undefined' : _typeof(el)) === 'object' && el.setAttribute, 'xDivTester.callWith(): Element provided is not a valid HTMLElement');
